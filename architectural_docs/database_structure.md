...
The database is structured into 5 main categories:
...
| Responses | SurveyResponse, FieldAnswer | Storing user submissions |
| RBAC | User, Role, UserRole, Permission, RolePermission | Access control |
...
| created_by | UUID | FK → User | User who created the survey |
...
| config | JSONB | NULLABLE | Additional configuration (see below) |
...
| source_field_id | UUID | FK → Field | The field whose answer triggers this rule |
...
| source_field_id | UUID | FK → Field | The field that triggers the change |
...
|--------|------|-------------|-------------|
| id | UUID | PK | Unique identifier |
| survey_id | UUID | FK → Survey | The survey being answered |
| respondent_id | UUID | FK → User, NULLABLE | If user is authenticated |
...
| last_section_id | UUID | FK → Section, NULLABLE | For resuming partial responses |
...
**Constraints:**
- CHECK: At least one of `respondent_id` or `session_token` must be NOT NULL
...
| field_id | UUID | FK → Field | The field being answered |
...
| answered_at | TIMESTAMP | NOT NULL, DEFAULT NOW() | When this answer was saved |
...
- CHECK: Exactly one of `value` or `encrypted_value` should be NOT NULL
...
| action | VARCHAR(50) | NOT NULL | One of: `created`, `updated`, `deleted`, `viewed`, `exported` |
| resource_type | VARCHAR(50) | NOT NULL | One of: `survey`, `section`, `field`, `response` |
...
- Encryption at application layer using AES-256
...
2. Use `django-encrypted-model-fields` or similar for field-level encryption
3. Create initial data migration for default roles and permissions
4. Consider using `django-simple-history` for model versioning (optional)
